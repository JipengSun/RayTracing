...
CScene.prototype.makeRayTracedImage = function(AAcode,isJitter) {
//==============================================================================
...
//	console.log("You called CScene.makeRayTracedImage!")
  ...
  var i,j,m,n;      // pixel x,y coordinate (origin at lower left; integer values)
  ...
  for(j=0; j< this.imgBuf.ySiz; j++) {        // for the j-th row of pixels.
      ...
      for(m=0;m< AAcode; m++){
				for(n=0;n< AAcode;n++){
          if(isJitter){
						this.rayCam.setEyeRay(this.eyeRay,i+(m+Math.random())*subgap,j+(n+Math.random())*subgap);
					}
					else{
